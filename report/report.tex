...
resolution parameters (0.02, 0.05, 0.1, 0.25, and 0.5) to check whether 
independent clustering would recapture the known regional and villus 
layer structure annotated by the authors. Clustering at resolution = 0.5 
yielded 27 clusters that are largely region-pure (Figure 
\ref{fig:leiden_heatmap}), but each region was split into multiple 
sub-clusters corresponding to the ``petal'' substructure of the UMAP, 
not to the known villus layers. At the lowest resolution (0.02), the 
clustering produced 4 clusters — the same number as the annotated 
regions — but these clusters were not pure, with the two jejunum 
regions mixing the most. This suggests that both the proximal-distal 
axis and, even more so, the crypt-to-villus axis form a continuous 
gradient, and that spatially adjacent regions cannot be easily 
separated by unsupervised clustering. For this reason, I used the 
pre-existing expert annotations (tissue region and ylayer) directly 
for the differential expression analysis.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{03_cluster_composition_heatmap.png}
    \caption{Composition of Leiden clusters (resolution=0.5) with respect 
    to tissue region (A) and villus layer (B). Each row shows, for one 
    Leiden cluster, the fraction of its spots belonging to each category. 
    Most clusters are dominated by a single region (Panel A), with the 
    two jejunum regions showing the most mixing, while no cluster is 
    dominated by a single villus layer (Panel B).}
    \label{fig:leiden_heatmap}
\end{figure}
...
include \textit{Reg1} — critical for tissue regeneration and 
\textit{S100g} — coding calcium binding protein G; jejunum markers include 
\textit{Fabp1} — coding fatty acid-binding protein and \textit{Apoa4} —
respondible for transport of fats and cholersterol; and ileum 
markers include \textit{Fabp6} — encoded ileal bile acdid-binding protein
and \textit{Defa22} — encoding a defensin(Figure \ref{fig:dotplot_region}).
...
\textit{Olfm4} (intestinal stem cell marker encoding a glycoprotein 
that plays critical roles in inflammation and cellular proliferation), 
and \textit{Stmn1} (expressed in rapidly dividing progenitors), 
validating the biological accuracy of the layer annotations 
...
The Bottom villous SI is the most microbiome-promoted 
layer across three out of four of the SI regions (3,142-6,600 
SPF-upregulated DEGs), suggesting the microbiome primarily 
influences the enterocyte maturation zone rather than stem 
cell identity or fully differentiated cells.
...
expression that increases toward the villus tip. However, this might 
be due to the fact, that mature enterocytes are in general more diverse 
than the cells in the crypt.The conserved 
...
bile acids via specific transporters. This is overall consistent with 
prior knowledge.
...
greatest microbiome sensitivity. This hypothesis was not fully confirmed. 
...
and with evidence from gnotobiotic mouse models that microbiota 
presence shapes small intestinal villus structure and development 
\cite{paeslack2024impact}.
...
requires validation in human samples (e.g. \textit{Defa22} gene 
in mice has similar functionality to human's \textit{DEFA5}). 
Third, the microbiome comparison is limited to SPF vs GF; the 
FMT condition, which would allow assessment of microbiome 
rescue effects, was not analyzed due to time constraints. 
Fourth, the number of DEGs detected is potentially influenced 
by spot count even after downsampling, as larger absolute 
numbers of spots per condition increase statistical power 
for detecting small effect sizes. Finally, it would be 
undoubtedly benefitial to contextualize the villus tip 
markers identified here against published transcriptomic 
datasets from intestinal disease models — such as celiac 
disease, where villous atrophy selectively destroys the 
tip compartment — to directly quantify which conserved 
tip programs are disrupted during pathology.
...
provide insights on the technical side of the code, and to help with the
